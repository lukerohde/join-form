class JoinFormsController < ApplicationController
  ...
  private
    ...
    def join_form_params
      result = params.require(:join_form).permit(:short_name, :description, :advanced_designer, :css, :header, :footer, :wysiwyg_header, :wysiwyg_footer, :bootsy_image_gallery_id, :credit_card_on, :direct_debit_on, :payroll_deduction_on, :direct_debit_release_on, :page_title, :base_rate_id, :base_rate_establishment, :base_rate_weekly, :base_rate_fortnightly, :base_rate_monthly, :base_rate_quarterly, :base_rate_half_yearly, :base_rate_yearly, :union_id, :person_id, :column_list, :signature_required, :welcome_email_template_id, :admin_email_template_id, :message_types => [])
