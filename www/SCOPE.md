...
### Part 1 - Get it done
When there are no custom questions in the custom questions it does not show TESTED.  In the payment step the user is presented with a reasonable set of default options, in the following order - pay method (DD or CC), Frequency (fortnightly), deduction date (starting today unless DD, then next working day).  If the user changes pay method, frequency or deduction date the payment step is refreshed, keeping their input where possible, but only presenting valid input choices TESTED - for instance, it doesn't make sense to have today as the first day of payment for direct debit TESTED.  Choosing a weekly payment doesn't allow deferred payment beyond a week.  TESTED Choosing a fortnightly option doesn't allow deferring beyond more than a fortnight TESTED.  Choosing a monthly payment doesn't allow selecting a date beyond one month TESTED. Choosing an quarterly or annual payment option may not show any deferred payment option at all TESTED.  Choosing a pay roll deduction option doesn't show the frequency or deferred payment options.  Choosing dd release, allows frequency and shows a notice explaining payment will start after dd release.  Changing a frequency will change the amount payable now. TESTED Deferring a payment will defer the charge to the user and will only validate the credit card rather than charge it.  Subscription description will show somewhere sensible.  Translations for all text changes (including subscription to miscellaneous rename, deduction_date).
