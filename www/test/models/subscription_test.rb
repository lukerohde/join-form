...
class SubscriptionTest < ActiveSupport::TestCase
  ...
  test "step 4 - pay method validation fail" do
    s = subscriptions(:contact_details_with_subscription_subscription)
    assert s.save == false
    assert s.errors.messages == {:pay_method=>["must be specified"]}

    assert s.step == :pay_method
  end

  test "step 4 - pay method validation fail - ABR" do
    s = subscriptions(:contact_details_with_subscription_subscription)
    s.pay_method = "AB"
    assert s.save == false
    assert s.errors.messages == {
  ...
  test "step 4 - pay method validation fail - CC" do
    ...
    assert s.errors.messages == {
      :card_number=>
  ["couldn't be validated by our payment gateway.  Please try again."]
    }
  ...
  test "step 2 - address validation pass" do
    s = subscriptions(:contact_details_only_subscription)
    #binding.pry
  ...
  test "step 3 - miscellaneous validation pass" do
    s = subscriptions(:contact_details_with_address_subscription)
    #binding.pry
    assert s.update(plan: "adsf", frequency: "F")
    assert s.step == :pay_method
    s.reload
    assert s.step == :pay_method
  end
  ...
  test "step 4 - pay_method validation australian bank pass" do
    ...
    assert s.update!(pay_method: "AB", bsb: "123-123", account_number: "123456", partial_account_number: "123xxx")
  ...
  test "step 4 - pay_method validation credit card pass" do
    ...
    assert s.update_with_payment({pay_method: "CC", stripe_token: "asdf", partial_card_number: "xxxxxxxxxxxxx123"}, s.join_form.union)
